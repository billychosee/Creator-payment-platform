...
export function Dashboard({ metrics, onWithdraw }: DashboardProps) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          >
            ...
            <div className="relative">
              ...
              <p className="text-blue-400 text-xs mt-2">Lifetime earnings</p>
            </div>
          </div>
          ...
          >
            ...
            <div className="relative">
              ...
              <p className="text-purple-400 text-xs mt-2">Last 30 days</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          >
            <div className="flex items-center gap-4">
              <div className="w-14 h-14 bg-gradient-to-br from-red-600 to-pink-600 rounded-xl flex items-center justify-center">
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-1 gap-4">
            {metrics.topViewedVideos.map((video, index) => (
              <div
                key={index}
                className="group relative rounded-xl p-4 transition-all duration-300 cursor-pointer backdrop-blur-xl bg-white/10 border border-white/20 shadow-xl shadow-black/20"
                style={{
                  backgroundColor: "var(--app-card-bg)",
                  border: "1px solid var(--app-card-border)",
                }}
                onClick={() =>
                  setSelectedVideo({
                    id: video.id ?? `top-${index}`,
                    title: video.name,
                    views: video.views ?? 0,
                    createdAt: video.createdAt
                      ? new Date(video.createdAt)
                      : new Date(),
                    status: (video.status as any) ?? "active",
                    channelName: (video.channelName as any) ?? "Channel",
                    playlistName: (video.playlistName as any) ?? "",
                    thumbnail: videoThumbnails[index % videoThumbnails.length],
                  })
                }
              >
                <div className="flex items-center gap-4">
                    ...
                    <div
                      className={`w-12 h-12 rounded-lg flex items-center justify-center text-2xl ${
                        index === 0
                          ...
                          : index === 2
                          ? "bg-gradient-to-br from-orange-800 to-orange-900 text-white"
                  ...
                  <div className="flex-1 min-w-0">
                    <h3
                      className="text-lg mb-1 truncate group-hover:text-red-500 transition-colors"
                      style={{ color: "var(--app-text)" }}
                    >
                      {video.name}
                    </h3>
                  ...
                  <div
                    style={{
                      width: animateProgress
                        ? `${
                            ((video.views ?? 0) /
                              (metrics.topViewedVideos[0]?.views ?? 1)) *
                            100
                          }%`
                        : "0%",
