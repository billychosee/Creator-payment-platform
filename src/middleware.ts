...
const BLOCKED_USER_AGENTS = [
  ...
  'python-requests', 'http-client', 'httpie', 'postman',
];

const BLOCKED_PATHS = [
  '/wp-admin', '/wp-login.php', '/admin', '/administrator',
  '/login', '/phpmyadmin', '/mysql', '/.env', '/.git',
  '/config', '/api/config',
];
...
export function middleware(request: NextRequest) {
    ...
    const response = NextResponse.json(
      ...
      { 
        ...
        headers: {
          ...
          'X-RateLimit-Remaining': Math.max(0, RATE_LIMIT_REQUESTS - rateLimitResult.count).toString(),
  ...
  if (containsSQLInjection(request.nextUrl.searchParams) || 
...
function checkRateLimit(ip: string, method: string): { allowed: boolean; count: number; resetTime: number } {
  const now = Date.now();
  const key = `${ip}:${method}`;
...
function createSecurityResponse(message: string, status: number, startTime: number): NextResponse {
  const response = NextResponse.json(
    {
      error: message,
